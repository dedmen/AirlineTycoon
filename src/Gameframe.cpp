...
#include "Intro.h"
#include "Outro.h"
#include "NewGamePopup.h"  //Fenster zum Wahl der Gegner und der Spielst�rke
#include "cd_prot.h"
#include "AtNet.h"
...
void GameFrame::PrepareFade(void)
{
   ...
   //Erkl�rung, bei der Kopie dieses Code-Fragments...
   if (!bLeaveGameLoop)
   {
      SLONG c=0;
...
void GameFrame::OnPaint()
{
         ...
         else
         {
            ...
            else if (ToolTipState==FALSE && timeGetTime()-ToolTipTimer>600)
            {
               ...
               if (ToolTipId>0)
               {
                  ...
                  if (str=="Triebwerke ausw�hlen") str="Scheinwerfer ausw�hlen";
               }
               else if (ToolTipId<0)
                  str=ToolTipString;
   ...
   if (bActive && Sim.bPause)
   {
      ...
      if (PauseFade==0)
      {
         ...
         //per memcpy, dann geht alles wunderbar. Der Bug trat �brigens nur im Fenster-
...
void GameFrame::OnActivateApp(BOOL bActive, DWORD hTask)
{
   if (::bActive != bActive)
   {
      ...
      if (bActive)
      {
         ...
         {
            //Gro�alarm, wegen Fullscreen: Zahlreiche Bitmaps m�ssen restauriert werden:
...
void GameFrame::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
   static char TypeBuffer[30];   //F�r Cheats
   long nTargetRoom=0;
   ...
   if (nChar>='A' && nChar<'Z')
   {
          ...
          TypeBuffer[29]=='P')
      {
         ...
         Sim.Players.Players[Sim.localPlayer].ArabTrust  = 6; //F�r Spieler 2

         /*Sim.Players.Players[Sim.localPlayer].ArabMode2  = 1; //Bakterien im Kaffee
         Sim.Players.Players[Sim.localPlayer].ArabOpfer2 = 2; //F�r Spieler 2
         Sim.Players.Players[Sim.localPlayer].ArabTrust  = 6; //F�r Spieler 2
...
void GameFrame::OnLButtonDown(UINT nFlags, CPoint point)
{
   //Alles blockieren, wenn im Optionsmen�:
   if (nOptionsOpen && Sim.bNetwork && Sim.localPlayer<Sim.Players.Players.AnzEntries() && Sim.Players.Players[Sim.localPlayer].GetRoom()!=ROOM_OPTIONS && Sim.Players.Players[Sim.localPlayer].GetRoom()!=0) return;
...
//--------------------------------------------------------------------------------------------
//Aktiviert einen Tool-Tip wobei eine TextString �bergeben wird. 
//--------------------------------------------------------------------------------------------
