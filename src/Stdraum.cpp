...
//Also wird gepacht und �ber diese Variablen der Klick ans erste Fenster geleitet...
...
//--------------------------------------------------------------------------------------------
//Sort daf�r, da� die Statusanzeige erneuert wird:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Gibt die Nummer anhand des Id's zur�ck:
//--------------------------------------------------------------------------------------------
...
CStdRaum::CStdRaum (BOOL bHandy, ULONG PlayerNum, CString GfxLibName, __int64 graficId)
{
   ...
   CStdRaum::BubbleStyle   = 2;              //Von S�den
   ...
   //Und noch 'ne Anomalie. Der Player h�lt 'ne Kopie der Fenster-Koordinaten
   if (PlayerNum!=-1)
   {
...
//--------------------------------------------------------------------------------------------
//Gibt zur�ck, ob zur Zeit ein Dialog offen ist:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//L��t ein neues Text-Fenster auf dem Schirm erscheinen:
//--------------------------------------------------------------------------------------------
void CStdRaum::MakeSayWindow (BOOL TextAlign, ULONG SubId, const CString &String, SB_CFont *Normal)
{
   ...
   if (TextAlign)
   {
      ...
      if (Sim.Players.Players[PlayerNum].GetRoom()==ROOM_BURO_A+PlayerNum*10 && TextAlign!=0) //Im B�ro?
...
//--------------------------------------------------------------------------------------------
//L��t ein neues Auswahl Text-Fenster auf dem Schirm erscheinen:
//--------------------------------------------------------------------------------------------
void CStdRaum::MakeSayWindow (BOOL TextAlign, const char *GroupId, ULONG SubIdVon, ULONG SubIdBis, BOOL ParameterIndiziert, SB_CFont *Normal, SB_CFont *Highlight, ...)
{
   ...
   for (c=CurrentTextSubIdVon; c<=(SLONG)CurrentTextSubIdBis; c++)
   {
      ...
      //Sind die Paramter eine Liste f�r die Antworten oder jeweils alle f�r alle?
...
void CStdRaum::MakeNumberWindow (CString Text)
{
   ...
   else
   {
      ...
      //Auf dem Bildschirm Platz f�r das Zahlenfeld suchen:
...
void CStdRaum::RepaintText (BOOL RefreshAll)
{
   ...
   if (CurrentTextGroupId)
   {
      ...
      else
      {
         ...
         if (TextAlign!=0) //unten, vom Spieler
         {
            ...
            {
               if (DialogPartner==TALKER_COMPETITOR) //Bei Dialog mit anderen Spielern rechts mehr Platz lassen
                  if (Sim.Players.Players[PlayerNum].GetRoom()==ROOM_BURO_A+PlayerNum*10) //Im B�ro?
                  {
         ...
         else //oben, vom gespr�chspartner
         {
   ...
   {
      //Dadurch bricht die Transparenzroutine (falls sie gerade l�uft) ab.
      gRoomJustLeft = TRUE;
...
void CStdRaum::InitToolTips (void)
{
   ...
   if (bHandy==FALSE)
   {
      ...
      //Die ToolTips f�r die Statuszeile: Konto, Logo, Speed, Zeit
      if (TopWin==NULL)
      {
      ...
      //Ist der Cursor irgenwo in einem Men�?
      if (MenuIsOpen())
      {
         CursorPos-=MenuPos;
...
//--------------------------------------------------------------------------------------------
//Malt ggf. �ber den gesammten Raum noch den Text dr�ber:
//--------------------------------------------------------------------------------------------
void CStdRaum::PostPaint (void)
{
   ...
   if (bHandy==FALSE)
   {
      ...
      if (bActive && CurrentTextGroupId && (bHandy==FALSE || TempScreenScrollV==0))
      {
         ...
         if (pSmackerPartner==NULL || TextAlign!=0 || pSmackerPartner->GetMood()==SPM_TALKING || timeGetTime()>DWORD(SmackerTimeToTalk))
         {
            ...
            //Den Cursor-Highlight �berpr�fen:
   ...
   if (bActive && Sim.Gamestate==(GAMESTATE_PLAYING | GAMESTATE_WORKING))
   {
      ...
      if (DialogPartner==TALKER_COMPETITOR)
      {
         ...
         //Gedankenblase f�r nachdenkende Netzwerkspieler:
...
//--------------------------------------------------------------------------------------------
//�berpr�ft, ob der Cursor mitlerweile auf einem anderem Highlight steht:
//--------------------------------------------------------------------------------------------
void CStdRaum::CheckHighlight (CPoint point)
{
   //Au�erhalb geklickt? Dann finden wir eh' nichts!
   ...
   //Falls wir hier ein Auswahlmen� haben:
...
void CStdRaum::OnPaint(BOOL bHandyDialog)
{
...

   {
      ...
      {
         //Alten Handy-Schirm ggf. zerst�ren?
...
//--------------------------------------------------------------------------------------------
//Mitteilung, da� Daten aktualisiert wurden. Ggf. mu� der Tip neugezeichnet werden:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Konvertiert die Koordinaten auf einheitlich 640x440, FALSE wenn au�erhalb
//--------------------------------------------------------------------------------------------
...
void CStdRaum::MenuStart (SLONG MenuType, SLONG MenuPar1, SLONG MenuPar2, SLONG MenuPar3)
{
   ...
   //Ein Men� unterbricht den Dialog; wenn man danach in den Dialog zur�ckkehrt, dann bleibt der Dialogpartner interessiert:
   if (IsDialogOpen())
   {
   ...
   switch (CurrentMenu)
   {
      ...
      case MENU_KONTOAUSZUG:
         ...
         OnscreenBitmap.ReSize (MenuBms[0].Size);
         MenuPage=9;  //Seite 9: Eintr�ge 90-99; Seite 8: Eintr�ge 80-89; ...
         break;
   ...
   switch (CurrentMenu)
   {
      ...
      case MENU_CLOSED:
         OnscreenBitmap.BlitFrom (MenuBms[0]);
         {
            ...
            char *p = strtok ((char*)(LPCTSTR)str, "�");
            SLONG y = 20;
         ...
         //Texte reinschreiben:
         {
            ...
            for (pass=1; pass<48; pass++)
            {
               ...
               for (c=0; c<Sim.Players.AnzPlayers; c++)
                  if (!Sim.Players.Players[c].IsOut)
                  {
                     //F�r alle Flugzeuge des Spielers:
...
//--------------------------------------------------------------------------------------------
//Nachtr�gliche Menu-Ver�nderungen, wie z.B. Icon Highlights
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Berichtet, ob zur Zeit ein Men� offen ist:
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
//Reaktion auf linken Mausklick im Men�:
//--------------------------------------------------------------------------------------------
      ...
      case MENU_SELLPLANE:
         ...
         else if (Pos.IfIsWithin (216,6, 387,212))
         {
            ...
            if (n>=0 && n<MenuDataTable.AnzRows && qPlayer.Planes.IsInAlbum (MenuDataTable.LineIndex[n]))
            {
               ...
               //Das Flugzeug kann nur verkauft werden, wenn keine Fl�ge darauf gebucht sind:
         ...
         if (Sim.bNetwork)
         {
            ...
            Sim.bNetwork = false;
              ...
              if (Sim.Difficulty==DIFF_FINAL   && MenuPar1==0) Sim.Gamestate = UBYTE(GAMESTATE_OUTRO);
                                                          ...
                                                          else Sim.Gamestate = GAMESTATE_BOOT;      //Ins Hauptmen�!
         break;
...
//--------------------------------------------------------------------------------------------
//Reaktion auf rechten Mausklick im Men�:
//--------------------------------------------------------------------------------------------
void CStdRaum::MenuRightClick (XY)
{
   ...
   //Fraglich wozu das gut war; F�hrte aber zu problemen mit dem Sabotage-Dialog
...
//--------------------------------------------------------------------------------------------
//�bergibt die Daten f�r das Zoommen�:
//--------------------------------------------------------------------------------------------
...
void CStdRaum::OnChar(UINT nChar, UINT, UINT) 
{
   ...
   {
      if (nChar==' ' || nChar==':' || nChar==',' || nChar==';' || nChar=='!' || nChar=='?' || nChar=='\'' || nChar=='-' || nChar=='+' || nChar=='(' || nChar==')' || (nChar>='0' && nChar<='9') || (nChar>='A' && nChar<='Z') || (nChar>='a' && nChar<='z') || nChar==UBYTE('�') || nChar==UBYTE('�') || nChar==UBYTE('�') || nChar==UBYTE('�') || nChar==UBYTE('�') || nChar==UBYTE('�') || nChar=='.' || nChar==UBYTE('�'))
      {
...
//--------------------------------------------------------------------------------------------
//Der Raum der als n�chstes geladen wird, �bernimmt die Roomlib; sie wird nicht vernichtet!
//--------------------------------------------------------------------------------------------
